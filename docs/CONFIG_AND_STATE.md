...
| `refresh_interval_min` | widget: fixed 30s; TV: fixed 20s (both hardcoded, not configurable) | int | **New as a user-facing setting** — neither existing app exposes this; free-tier API math forces it to minutes, not seconds, on the firmware (see `DEPLOYMENT_OPS.md`) |
