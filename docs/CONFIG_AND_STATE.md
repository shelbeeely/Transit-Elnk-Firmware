...
| `tz` | n/a — both reference apps run in a browser that already knows the local time zone | string | **New setting.** POSIX TZ string, default `PST8PDT,M3.2.0,M11.1.0` (Pacific with US DST). Decides which GTFS service day is "today", whether a departure has passed, and the sleep window's notion of overnight. Before this existed the firmware ran on UTC — see `docs/OFFLINE_AND_BUS_WIFI.md` |
